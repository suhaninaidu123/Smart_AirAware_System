...
async function findPune() {
  try {
    ...
    const locUrl = `https://api.openaq.org/v3/locations?bbox=${bbox.join(',')}&limit=5`;
    const locRes = await fetch(locUrl, { headers });
    const locJson = await locRes.json();
    
    if (locJson.results && locJson.results.length > 0) {
        console.log('Result 0:', JSON.stringify(locJson.results[0], null, 2));
    } else {
        console.log('No locations found in bbox.');
    }
  } catch (error) {
